...
test('berry reed starts small, grows with hygiene, and boosts spawn throughput over time', () => {
  const world = makeWorldForTest();
  world.birthsCount = 8;
  world.water.hygiene01 = 1;
  assert.equal(world.addBerryReedPlant().ok, true);
  ...
  const plant = world.berryReedPlants[0];
  const initialHeight = plant.height;
  const initialInterval = plant.nextFruitAtSec - world.simTimeSec;
  assert.ok(initialHeight < world.bounds.height * 0.16, `expected a small initial plant, got ${initialHeight}`);
  assert.ok(Math.abs(initialInterval - 24) < 0.001, `expected initial ideal interval ~24s, got ${initialInterval}`);
  ...
  assert.ok(plant.growth01 > 0.99, `expected full growth around 2 juvenile windows, got ${plant.growth01}`);
  assert.ok(plant.height > initialHeight, 'expected height growth over time');

  world.water.hygiene01 = 1;
  world.fruits = [];
  plant.nextFruitAtSec = world.simTimeSec - 0.001;
  withStubbedRandom(0.5, () => world.update(0.2));
  const grownInterval = plant.nextFruitAtSec - world.simTimeSec;
  assert.ok(Math.abs(grownInterval - 8) < 0.001, `expected grown ideal interval ~8s, got ${grownInterval}`);
});

test('berry reed growth and fruit cadence pause below hygiene threshold and resume above it', () => {
  const world = makeWorldForTest();
  world.birthsCount = 8;
  world.water.hygiene01 = 1;
  assert.equal(world.addBerryReedPlant().ok, true);
  ...
  const plant = world.berryReedPlants[0];
  world.update(1200);
  const growthAfterGoodWater = plant.growth01;

  world.water.hygiene01 = 0.3;
  world.update(1200);
  assert.ok(Math.abs(plant.growth01 - growthAfterGoodWater) < 0.000001, 'growth should stop when hygiene is below 0.4');

  world.fruits = [];
  plant.nextFruitAtSec = world.simTimeSec - 0.001;
  withStubbedRandom(0.5, () => world.update(0.2));
  assert.equal(plant.nextFruitAtSec, Infinity);

  world.water.hygiene01 = 0.9;
  withStubbedRandom(0.5, () => world.update(0.2));
  assert.ok(Number.isFinite(plant.nextFruitAtSec), 'spawn timer should resume after hygiene recovers');
});

test('berry reed fruit slot cap scales with plant growth', () => {
  const world = makeWorldForTest();
  world.birthsCount = 8;
  world.water.hygiene01 = 1;
  assert.equal(world.addBerryReedPlant().ok, true);
  ...
  const makeFruit = () => ({
    ...
    v: 0,
    radius: 2,
    createdAtSec: world.simTimeSec,
    ttlSec: 999
  });

  plant.growth01 = 0;
  world.fruits = [makeFruit(), makeFruit(), makeFruit(), makeFruit()];
  plant.nextFruitAtSec = world.simTimeSec - 0.001;
  withStubbedRandom(0.5, () => world.update(0.2));
  assert.equal(world.fruits.length, 4, 'young plant should cap at 4 simultaneous fruits');

  plant.growth01 = 1;
  plant.height = plant.maxHeight;
  plant.nextFruitAtSec = world.simTimeSec - 0.001;
  withStubbedRandom(0.5, () => world.update(0.2));
  assert.equal(world.fruits.length, 5, 'mature plant should allow more than 4 simultaneous fruits');
});
