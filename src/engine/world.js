...
export class World {
  ...
  serialize() {
    return {
      ...
      fish: this.fish.map((f) => f.toJSON()),
      food: this.food.map((p) => ({ ...p })),
      bubbles: this.bubbles.map((b) => ({ ...b }))
    };
  }

  static deserialize(width, height, data) {
    const world = new World(width, height, 0);
    // Keep computed sandHeight based on current height
    world.simTimeSec = data.simTimeSec ?? 0;
    world.nextFoodId = data.nextFoodId ?? 1;
    world.nextFishId = data.nextFishId ?? 1;
    world.selectedFishId = data.selectedFishId ?? null;
    world.paused = data.paused ?? false;
    world.speedMultiplier = data.speedMultiplier ?? 1;

    world.food = Array.isArray(data.food) ? data.food.map((p) => ({ ...p })) : [];
    world.bubbles = Array.isArray(data.bubbles) ? data.bubbles.map((b) => ({ ...b })) : world.bubbles;

    world.fish = Array.isArray(data.fish)
      ? data.fish.map((f) => Fish.fromJSON(world.bounds, f))
      : [];

    // Ensure bounds applied
    for (const fish of world.fish) fish.setBounds(world.bounds);
