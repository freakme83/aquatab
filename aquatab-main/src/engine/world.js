...
export class World {
  ...
  serialize() {
    return {
      ...
      nextFishId: this.nextFishId,
      selectedFishId: this.selectedFishId,
      paused: this.paused,
      speedMultiplier: this.speedMultiplier,
      water: this.water,
      fish: this.fish.map((f) => (typeof f.toJSON === 'function' ? f.toJSON() : null)).filter(Boolean),
  ...
  static deserialize(width, height, payload) {
    try {
      const state = typeof payload === 'string' ? JSON.parse(payload) : payload;
      if (!state || typeof state !== 'object') return null;

      // Build a world with the right bounds, then hydrate.
      const w = new World(width, height, 1);
      w.fish = [];
      w.food = [];
      w.poop = [];
      w.eggs = [];
      w.nextFoodId = Number.isFinite(state.nextFoodId) ? state.nextFoodId : 1;
      w.nextFishId = Number.isFinite(state.nextFishId) ? state.nextFishId : 1;
      w.simTimeSec = Number.isFinite(state.simTimeSec) ? state.simTimeSec : 0;
      w.selectedFishId = Number.isFinite(state.selectedFishId) ? state.selectedFishId : null;
      w.paused = Boolean(state.paused);
      w.speedMultiplier = Number.isFinite(state.speedMultiplier) ? state.speedMultiplier : 1;
      w.water = state.water && typeof state.water === 'object' ? { ...w.water, ...state.water } : w.water;

      if (Array.isArray(state.food)) {
        w.food = state.food
          .map((f) => ({
            id: f.id,
            x: clamp(f.x ?? 0, 0, w.bounds.width),
            y: clamp(f.y ?? 0, 0, w.#swimHeight()),
            amount: Math.max(0.1, f.amount ?? FOOD_DEFAULT_AMOUNT),
            ttl: f.ttl ?? FOOD_DEFAULT_TTL,
            vy: f.vy ?? rand(8, 20)
          }))
      ...
      if (Array.isArray(state.fish)) {
        w.fish = state.fish
          .map((data) => {
            try {
              const fish = Fish.fromJSON?.(w.bounds, data);
              if (fish) {
                // Make sure derived growth/size fields match current simTime.
                fish.updateLifeCycle?.(w.simTimeSec);
                return fish;
              }
              return null;
            } catch {
      ...
      if (!w.fish.some((f) => f.id === w.selectedFishId)) {
        w.selectedFishId = w.fish[0]?.id ?? null;
      }

      // Bubbles are purely cosmetic; rebuild.
      w.#seedBubbles();
      return w;
    } catch {
      return null;
